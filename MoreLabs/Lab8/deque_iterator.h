 ...
 * dagoldstein@scu.edu
 * Lab 8
 * Description: Builds the deque iterator used for going through the deque
 * 
*/
...
namespace coen79_lab8
{
    ...
    public:
        ...
        { 
            assert(cursor != NULL && "Dereferencing a null iterator!");
            return *cursor;
             }
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            if (cursor == current_boundary)
            {
                ...
                if (current_block_pointer > block_pointers_end || *current_block_pointer == NULL)
                {
                    cursor = NULL;
                    current_boundary = NULL;
                    current_block_pointer = NULL;
